...
def _scene_text(
    project: Project, texture_res_paths: dict[str, str], script_res_path: str
) -> tuple[str, dict[str, str]]:
    template = get_rig_template(project.rig_profile)
    layers = ordered_layers(project.assembly_layers)
    ext = [f'[ext_resource type="Script" path="{script_res_path}" id="1_script"]']
...
def _verification_script(
    scene_res_path: str, preview_res_path: str, expected: list[dict[str, object]]
) -> str:
...
def export_godot_rig(
    ...
    try:
        ...
        scene_text, node_names = _scene_text(
            project, texture_res_paths, f"{output_res_directory}/script.gd"
        )
        ...
        (staging / "verify_rig.gd").write_text(
            _verification_script(scene_res_path, preview_res_path, expected),
            encoding="utf-8",
            newline="\n",
        )
        if output.exists():
